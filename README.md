...
| --- |
| ![Aurelia GP brake balance stability](readme-assets/screenshots/34-aurelia-brake-balance-stability.png) |

| Differential corner exit |
| --- |
| ![Aurelia GP differential corner exit](readme-assets/screenshots/35-aurelia-differential-corner-exit.png) |
...
- Power-on understeer now loads the front tires when throttle arrives before the wheel is unwound, trimming steering authority, grip reserve, tire scrub, audio, haptics, camera, and HUD feedback from one shared signal.
...
- Tire thermal load now remembers overdriven rubber for a few moments, cooling through rain and lighter demand while feeding tire texture, haptics, HUD text, and renderer telemetry.
...
- Longitudinal-slip load now gives wheelspin, lockup, wet launches, and pedal mistakes one shared tire state, feeding tire relaxation, audio, haptics, HUD text, renderer telemetry, and wheel load.
...
- Aero yaw stall now catches crossed-up high-speed moments as a shared aero state, feeding HUD text, wind/tire texture, haptics, renderer telemetry, and subtle wing motion.
