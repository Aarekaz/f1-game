...
describe("buildFormulaCarProxy", () => {
  it("builds a fictional formula silhouette with wings, halo, and exposed wheels", () => {
    ...
    expect(Array.from(names).filter((name) => name.includes("wheel")).length).toBeGreaterThanOrEqual(18);
    expect(car.userData.disposableMaterials.length).toBeGreaterThanOrEqual(13);
  });
});
