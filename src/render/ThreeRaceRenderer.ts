...
export class ThreeRaceRenderer {
  ...
  update(telemetry: RaceTelemetry) {
    ...
    this.animateFormulaCar(this.car, {
      ...
      surfaceRumble: clamp(telemetry.surfaceRumble + telemetry.roadFeelFeedback * 0.34 + Math.abs(telemetry.splitSurfaceLoad) * 0.22, 0, 1),
    ...
    const cameraLongitudinalTarget =
      telemetry.phase === "ready" || podMode
        ? 0
        : clamp(accelerationCue * 1.35 - telemetry.car.braking * 1.18 + telemetry.aeroPlatformLoad * 0.72 + telemetry.car.slip * 0.34, -1.7, 1.85);
  ...
  private animateFormulaCar(
    ...
    for (const wheelName of ["front-left-wheel", "front-right-wheel", "rear-left-wheel", "rear-right-wheel"]) {
      ...
      const cornerLoad = clamp(tireLoad * 0.58 + suspensionCompression * 0.32 + frontLoad - side * lateralLoad * 0.55 + side * splitSurfaceLoad * 0.34, 0, 1);
