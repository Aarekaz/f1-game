...
export class HudController {
  ...
  private racecraftText(telemetry: RaceTelemetry) {
    ...
    if (telemetry.powerUndersteerLoad > 0.05) return `Power understeer ${(telemetry.powerUndersteerLoad * 100).toFixed(0)}%`;
    ...
    if (telemetry.loadTransferImpulse > 0.18) return `Load transfer ${(telemetry.loadTransferImpulse * 100).toFixed(0)}%`;
    ...
    if (telemetry.wheelHopLoad > 0.18) return `Wheel hop ${(telemetry.wheelHopLoad * 100).toFixed(0)}%`;
    ...
    if (telemetry.aeroYawStall > 0.18) return `Aero yaw ${(telemetry.aeroYawStall * 100).toFixed(0)}%`;
